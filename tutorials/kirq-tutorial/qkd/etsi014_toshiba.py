...
@app.cell(hide_code=True)
def _(mo):
    mo.image(".\qkd\images\qkd_etsi014.png")
    return
...
def _(mo):
    mo.md(
        ...
        - `KMSA_IP` and `KMSB_IP`: are the IP addresses of the Key Management System (KMS) of Alice and Bob. These addresses have been configured by the QKD owner and will depend on the network configuration in which the QKD systems are embedded (subnetwork, gateway, mask, DNS). 
        - `BOBSAE` and `BOBSAE`: are the Secure Access Exchange (SAE) identifiers of Alice and Bob, defined at the configuration of the QKD systems. This should be provided by the QKD owner.
...
def _(mo):
    mo.md(
        ...
        Now let's play with the API already, we are going to provide you with a list of information, and you will have to construct the correct links to provide to request, here is the table of data regarding the Toshiba 4.2MU system

        | Interface | Module | IP | SAE |
        | ---- | ---- | ---- | ---| 
        | KMS | Alice | 192.168.0.152 | alicesae |
        | KMS | Bob | 192.168.101.153 | bobsae |
        """
    )
    return
...
@app.cell
def _():
    # Exercise 1 : fill in the missing information to construct the proper links to access the web API for both Alice and Bob
    KMSA_IP = "192.168.0.152"
    KMSB_IP = "192.168.10.153"
    ALICESEA = "alicesae"
    BOBSEA = "bobsae"
    url_enc_key = f"https://{KMSA_IP}/api/v1/keys/{BOBSEA}/enc_keys"
    url_dec_key = f"https://{KMSB_IP}/api/v1/keys/{ALICESEA}/dec_keys"
    url_alice_status = f"https://{KMSA_IP}/api/v1/keys/{BOBSEA}/status"
...
def _(KMSA_IP, run_shell_command):
    ...
    1. To be able to access these adresses, you should first asks a user VPN access from Kirq's technical team, and then set-up your machine using the provided VPN credentials.
    ...
    # Exercice 2 : complete the following command, by providing the correct variable, in order to ping Alice and Bob's KMS interface :
...
def _(mo):
    mo.md(
        ...
        Below, we show an example of certificates that are currently deployed in one QKD system available in Kirq testbed, namely, the Toshiba 4.2 MU models. These certificates are located in `certificate/Toshiba`, inside this same repository. Observe in detail the certificates and try to familiarize yourself with the nomenclature, it is pretty straightforward.
        """
    )
    return
...
def _(run_shell_command):
    ...
    run_shell_command("dir .\\qkd\\certificates\\Toshiba\\") # For Windows
    # run_shell_command("ls ./qkd/certificates/Toshiba/") # For Linux
    return
...
def _(mo):
    mo.md(
        ...
        - **`Root CA` (Certificate Authority)** - `ca_crt.pem`: Serves as the trusted issuer and manager of digital certificates, establishing a secure chain of trust for all entities in the network.
        - **`Client Key`**  - `client_alice_key.pem` and `client_bob_key.pem`: The private key of a client, used for decrypting received messages and signing outgoing messages, ensuring secure client authentication and communication integrity.
        - **`Client Certificate`** - `client_alice.pem` and `client_bob.pem`: A digital certificate that contains the client’s public key and identity, used to authenticate the client to other network entities and enable encrypted communications.

        In practice, all these certificates have been configured on our QKD system, and when providing them with the API, it will prove that you are a legitimate user. 

        Practically, for a real application, you should set up the sender server with Alice certificates, and the receiver server with Bob's certificates.
...
@app.cell
def _(__file__, requests, url_alice_status):
    # Exercise 3 : provide the propper certificate to get status of Alice
    import os 

    current_dir = os.path.dirname(os.path.abspath(__file__))
    path_certs = os.path.join(current_dir, 'certificates', 'Toshiba')

    print(path_certs)

    root_ca = os.path.join(path_certs, 'ca_crt.pem') # In practice it is discarded because it is not a secure root ca.
    client_alice_cert = os.path.join(path_certs, 'client_alice_crt.pem')
    client_alice_key = os.path.join(path_certs, 'client_alice_key.pem')

    url_status = url_alice_status
    ...
    requests.exceptions.ConnectTimeout: HTTPSConnectionPool(host='192.168.0.152', port=443): Max retries exceeded with url: /api/v1/keys/ETSIB/status 
...
def _(mo):
    mo.md(
        ...
        Maybe you noticed that the code above does not use the `root_ca`, and that `verify=False`, the reason is because right now, 'ca_crt.pem` is a dummy certificate which is not secure, and we would need a trusted entity to verify the connection. 

        ## Get keys

        Alright, it's time to get keys from Alice, using all the knowledge we acquired during this tutorial, now it's your turn to provide the proper parameters and
        """
    )
    return
...
def _(enc_key, extract_key, key_id, os, path_certs, requests, url_dec_key):
    ...
    client_bob_cert = os.path.join(path_certs, 'client_bob_crt.pem')
    client_bob_key = os.path.join(path_certs, 'client_bob_key.pem')
    ...
    '''
    Try to rerun the code a second time, you will get an error. This is normal, the key has been consumed already, so it has been deleted from the KMS of the QKD pair!
    '''
    return (
        cert_bob,
        client_bob_cert,
